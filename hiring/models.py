...
class AreasConocimiento(models.Model):
    ...
    class Meta:
        managed = False  # Django no gestionará la creación/modificación de esta tabla en la DB
...
class Profesiones(models.Model):
    ...
    id_area_conocimiento = models.ForeignKey(AreasConocimiento, models.SET_NULL, db_column='ID_Area_Conocimiento', blank=True, null=True)
...
class Usuarios(models.Model):
    ...
    email = models.CharField(max_length=100, unique=True, help_text='Dato de login para todos los usuarios.')
    password = models.CharField(max_length=255, help_text='Debe almacenarse de forma encriptada (hash).')
    tipo_usuario = models.CharField(max_length=15, choices=TipoUsuario.choices, help_text='Discriminador para identificar el tipo de usuario.')
    fecha_creacion = models.DateTimeField(auto_now_add=True) # Django maneja automáticamente el DEFAULT CURRENT_TIMESTAMP
    estatus = models.CharField(max_length=10, choices=EstatusUsuario.choices, default=EstatusUsuario.ACTIVO)
...
class Empresas(models.Model):
    ...
    id_empresa = models.OneToOneField(Usuarios, models.CASCADE, primary_key=True, db_column='ID_Empresa', help_text='Clave primaria y foránea que referencia a Usuarios (relación 1-a-1).')
...
class Postulantes(models.Model):
    ...
    id_postulante = models.OneToOneField(Usuarios, models.CASCADE, primary_key=True, db_column='ID_Postulante', help_text='Clave primaria y foránea que referencia a Usuarios (relación 1-a-1).')
    ...
    id_universidad = models.ForeignKey(Universidades, models.SET_NULL, db_column='ID_Universidad', blank=True, null=True)
...
class Vacantes(models.Model):
    ...
    id_profesion = models.ForeignKey(Profesiones, models.SET_NULL, db_column='ID_Profesion', blank=True, null=True)
    fecha_publicacion = models.DateTimeField(auto_now_add=True) # Django maneja automáticamente el DEFAULT CURRENT_TIMESTAMP
    estatus = models.CharField(max_length=10, choices=EstatusVacante.choices, default=EstatusVacante.ACTIVA)
...
class Postulaciones(models.Model):
    ...
    id_postulante = models.ForeignKey(Postulantes, models.CASCADE, db_column='ID_Postulante')
    # Relación con Vacantes
    id_vacante = models.ForeignKey(Vacantes, models.CASCADE, db_column='ID_Vacante')
    fecha_postulacion = models.DateTimeField(auto_now_add=True) # Django maneja automáticamente el DEFAULT CURRENT_TIMESTAMP
    estatus = models.CharField(max_length=12, choices=EstatusPostulacion.choices, default=EstatusPostulacion.RECIBIDA)
    ...
    class Meta:
        ...
        unique_together = (('id_postulante', 'id_vacante'),) # Un postulante solo puede aplicar una vez a la misma vacante.
...
class Contratos(models.Model):
    ...
    id_postulacion = models.OneToOneField(Postulaciones, models.DO_NOTHING, db_column='ID_Postulacion', unique=True, help_text='Un contrato se origina de una única postulación aceptada.') # DO_NOTHING porque la lógica de negocio debe evitar la eliminación en cascada si no hay contrato.
    ...
    # Relación con Bancos
    id_banco = models.ForeignKey(Bancos, models.SET_NULL, db_column='ID_Banco', blank=True, null=True)
    estatus = models.CharField(max_length=10, choices=EstatusContrato.choices, default=EstatusContrato.ACTIVO)
    ...
    def __str__(self):
        return f"Contrato {self.id_contrato} ({self.id_postulacion.id_postulante.nombres})"
...
class ExperienciasLaborales(models.Model):
    ...
    fecha_fin = models.DateField(blank=True, null=True, help_text='Nulo si es el trabajo actual.')
...
class Nominas(models.Model):
    ...
    fecha_generacion = models.DateTimeField(auto_now_add=True) # Django maneja automáticamente el DEFAULT CURRENT_TIMESTAMP
    estatus = models.CharField(max_length=10, choices=EstatusNomina.choices, default=EstatusNomina.GENERADA)
    ...
    class Meta:
        ...
        unique_together = (('id_empresa', 'mes', 'anio'),) # Solo puede existir una nómina por empresa para un mes y año específico.
...
class Recibos(models.Model):
    ...
    id_contrato = models.ForeignKey(Contratos, models.DO_NOTHING, db_column='ID_Contrato') # Usamos DO_NOTHING porque no tiene ON DELETE en la DB original.
    salario_base = models.DecimalField(max_digits=12, decimal_places=2)
    monto_deduccion_inces = models.DecimalField(max_digits=10, decimal_places=2, help_text='Calculado como el 0.5% del salario.')
    monto_deduccion_ivss = models.DecimalField(max_digits=10, decimal_places=2, help_text='Calculado como el 1% del salario.')
    comision_hiring_group = models.DecimalField(max_digits=10, decimal_places=2, help_text='Calculado como el 2% del salario.')
